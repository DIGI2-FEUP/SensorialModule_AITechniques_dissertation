...
'''
Considerar apenas amostras de boa soldadura
'''
...
'''
print(data.shape)
print("Removed by Penetration: " + str(len(data[data["Penetration"] < 0.1])))
data = data[data["Penetration"] >= 0.1]
print("Removed by Weld_Width: " + str(len(data[data["Weld_Width"] < 0.5])))
data = data[data["Weld_Width"] >= 0.5]
print("Removed by Undercut: " + str(len(data[data["Undercut"] > 0.25 * t1])))
data = data[data["Undercut"] <= 0.25 * t1]
print("Removed by Root_Concavity: " + str(len(data[data["Root_Concavity"] > 0.25 * t2])))
data = data[data["Root_Concavity"] <= 0.25 * t2]
print(data.shape)
...
save_directory = "../4_figures/dataset" + str(dataset) + "/good_weld"
...
file_path = '../5_results/dataset' + str(dataset) + '_cleaned_penetration.csv'
...
# .csv file to store the results
file_path = '../5_results/dataset' + str(dataset) + '_cleaned_weld_width.csv'
if not os.path.exists(file_path):
    columns = ["k", "knn", "svm", "ridge", "lasso", "linear", "rf", "xgb"]
    # Open the CSV file in append mode and write the new data
    with open(file_path, mode='a', newline='') as file:
